...
def predictor(stock_df, inputSec) :
    ...
    # 노란 그래프가 학습 데이터의 학습율
    # 파란 그래프가 실제 데이터의 학습율
    plt.plot(train_error_summary, 'gold', label="train_error")
    plt.plot(test_error_summary, 'b', label = "test_error")
    plt.legend()
    plt.xlabel('Epoch(x100)')
    plt.ylabel('Root Mean Square Error')

    plt.figure(2)
    plt.plot(testY, 'gold', label = "Y")
    plt.plot(test_predict, 'b', label = "Y_hat")
